...
static size_t GetChildIndex(size_t index, enum direction side)
{
    if (side == LEFT)
    {
        return 2 * index + 1;
...
static size_t GetParentAddress(size_t index)
{
    return (index - 1) / 2;
}
               ...
               size_t arr_size, void *param)
{
    ...
    {
        root_idx = index;
        parent_idx = GetParentAddress(index);
                 ...
                 size_t arr_size, void *param)
{
    ...
    {
        root_idx = index;
        l_child_idx = GetChildIndex(index, LEFT);
        r_child_idx = GetChildIndex(index, RIGHT);
