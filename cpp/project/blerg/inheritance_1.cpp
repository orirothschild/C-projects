...
{
public:
    Base() : m_a(8) {std::cout << "B::Ctor" << std::endl;}
...
{
public:
    Derived() : m_b() {std::cout << "X::Ctor" << std::endl;}
    ~Derived() {std::cout << "X::Dtor" << std::endl;}
    void Print1() const {std::cout << "X::Print1 X::m_b - " << m_b << std::endl;
                      Base::Print1();
...
{
    Base *b1 = new Base;
    Base *b2 = new Derived; // This is a legal & desirable! When? Why?
